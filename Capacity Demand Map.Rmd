...
## joined the dataset above with the data set that gave the early ed filter grouped by tract and summarized for early ed capacity
...
EECNEW5$GEOID <- as.character(EECNEW5$GEOID_tract)
EECNEW5$NAMELSAD <- as.character(EECNEW5$NAMELSAD_tract)

EECNEW5_desert <- EECNEW5 %>%
  mutate(EarlyEdProvider = ifelse(early_ed == "TRUE", 1, 0)) %>%
  group_by(GEOID, NAMELSAD) %>%
  summarize(EarlyEdTract = sum(EarlyEdProvider)) %>%
  mutate(EarlyEdChildCareDesert = ifelse(EarlyEdTract <= 0, "TRUE", "FALSE")) %>%
  filter(EarlyEdChildCareDesert == "TRUE")

EEC_childcare_desert <- merge(data.frame(EEC_desert), data.frame(EECNEW5_desert), by = "GEOID", all = TRUE)

EEC_childcare_desert2 <- merge(data.frame(EEC_childcare_desert), data.frame(EEC_geometry), by = "GEOID", all = TRUE)


View(EEC_childcare_desert2)
```
