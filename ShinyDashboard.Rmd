  ...
  tabItems(
    ...
    tabItem(tabName = "capacity",
            box(
              ...
              status = "warning",
              background = "blue",
              "Info about area", br(), "Info about group of interest"),
            tags$br(),
            leafletOutput("capacity_demand_plot")
            ),
...
server <- function(input, output) {
  output$capacity_demand_plot <- renderLeaflet({
    leaflet(percentage_under_6) %>%
      ...
      addPolygons(fillColor = ~pal(percent_working),
                  ...
                  dashArray = "3",
                  fillOpacity = 0.7) %>%
      addPolygons(data=boston_neighborhood, weight = 2, opacity = 1, fillOpacity=0,
                  stroke=TRUE, color="black", label=~Name) %>% #adding neighborhood data
