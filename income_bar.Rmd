...
neighborho_demand <- income_neighborhood %>%
  ...
  summarise(medincome = sum(estimate))
...
ggplot(plottable,aes(x=reorder(Name_neighborho,order), y = var, fill = name))+scale_y_continuous("Median Income($)",sec.axis = sec_axis(~./500000, name = "Ratio"))+coord_flip()+
  geom_bar(stat="identity",position="dodge") + ggtitle( "Childcare ratio vs. Median Income by neighborhood")+ xlab("Neighborhoods")+  ylab("Median Income($)") + geom_hline(yintercept=1)+theme(panel.background = element_rect(fill='white', colour='white'))+scale_fill_manual(values=cbPalette)
```
